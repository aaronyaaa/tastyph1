...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['profile_pics']) && $_FILES['profile_pics']['error'] == 0) {
        ...
        $fileName = $userId . "_store_" . time() . "." . pathinfo($_FILES["profile_pics"]["name"], PATHINFO_EXTENSION);
        ...
        if (getimagesize($_FILES["profile_pics"]["tmp_name"]) !== false) {
            if ($_FILES["profile_pics"]["size"] <= 5000000) {  // Max 5MB
                if (move_uploaded_file($_FILES["profile_pics"]["tmp_name"], $targetFile)) {
                    ...
                    echo "Error uploading store profile picture.";
                    exit();
                }
            } else {
    ...
    if (!empty($fieldsToUpdate)) {
        $sql = "UPDATE apply_seller SET " . implode(', ', $fieldsToUpdate) . " WHERE seller_id = ?";
        $values[] = $userId; // Add seller_id at the end
        ...
        if ($stmt->execute()) {
            header("Location: ../includes/manage_products.php"); // Redirect after successful update
            exit();
        } else {
            echo "Error updating store profile.";
            exit();
        }
    } else {
